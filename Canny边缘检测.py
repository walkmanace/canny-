...
def _save_outputs(base_path: str, edges_custom: np.ndarray, edges_cv2: Optional[np.ndarray]=None):
    stem, _ = os.path.splitext(os.path.basename(base_path))
    out_custom = f"{stem}_canny_custom.png"
    cv2.imwrite(out_custom, edges_custom)
    out_cv2 = None
    if edges_cv2 is not None:
        out_cv2 = f"{stem}_canny_opencv.png"
        cv2.imwrite(out_cv2, edges_cv2)
    return out_custom, out_cv2


def main():
    parser = argparse.ArgumentParser(description="Canny Edge Detection (custom + OpenCV compare)")
    parser.add_argument("images", nargs="*", help="Path(s) to input image(s)")
    parser.add_argument("--auto", action="store_true", help="Use automatic thresholds based on median")
    parser.add_argument("--low", type=float, default=None, help="Low threshold (0-255) for hysteresis")
    parser.add_argument("--high", type=float, default=None, help="High threshold (0-255) for hysteresis")
    parser.add_argument("--kernel-size", type=int, default=5, help="Gaussian kernel size (odd)")
    parser.add_argument("--gauss-sigma", type=float, default=1.2, help="Gaussian sigma")
    ...
    for image_path in targets:
        ...
        use_auto = args.auto or (args.low is None or args.high is None)
        edges_custom, mids = canny_custom(
            img,
        ...
        edges_cv2 = cv2.Canny(img, int(mids['low_threshold']), int(mids['high_threshold']))

        out_custom, out_cv2 = _save_outputs(image_path, edges_custom, edges_cv2)

        if not args.no_show:
            plot_all(img, mids, edges_custom, edges_cv2)

        print(f"[OK] {os.path.basename(image_path)} -> low={mids['low_threshold']}, high={mids['high_threshold']} | saved: {out_custom}{' , ' + out_cv2 if out_cv2 else ''}")
        summaries.append({
            'image': os.path.basename(image_path),
            'low': mids['low_threshold'],
            'high': mids['high_threshold'],
            'custom': out_custom,
            'opencv': out_cv2,
        })
    ...
    if summaries:
        print("\nSummary:")
        for s in summaries:
            print(f"- {s['image']}: low={s['low']} high={s['high']} | {s['custom']} | {s['opencv']}")
